...
export function AddEmployeeModal({ open, onClose }) {
  ...
  const [form, setForm] = useState({
    first_name: '', last_name: '', job_title: 'Technician',
    hire_date: new Date().toISOString().split('T')[0],
    phone: '', email: '', employment_status: 'active',
  });
  ...
  useEffect(() => {
    if (!open) {
      setStep(0);
      setForm({ first_name: '', last_name: '', job_title: 'Technician', hire_date: new Date().toISOString().split('T')[0], phone: '', email: '', employment_status: 'active' });
      setSalary({ payment_structure: 'monthly', salary_structure_id: '', basic_salary_override: '25000', effective_date: new Date().toISOString().split('T')[0] });
    } else if (salaryStructures.length > 0 && !salary.salary_structure_id) {
      setSalary(prev => ({
        ...prev,
        salary_structure_id: String(salaryStructures[0].salary_structure_id),
        basic_salary_override: String(salaryStructures[0].basic_salary || '25000'),
      }));
    }
  }, [open, salaryStructures]);
  ...
  const validateStep0 = () => {
    ...
    if (!form.hire_date) { toast.error('Hire date is required'); return false; }
    return true;
  };
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const empId = newEmployee?.employee?.employee_id ?? newEmployee?.employee_id ?? newEmployee?.id;

      if (empId && salary.basic_salary_override) {
        try {
          ...
          toast.warning('Employee created, but salary assignment encountered an issue.', 'Salary Setup');
        }
      }
      ...
      toast.success(`${form.first_name} ${form.last_name} added successfully (${salary.payment_structure.toUpperCase()} ETB ${Number(salary.basic_salary_override || 0).toLocaleString()})`);
      onClose();
    } catch (err) {
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-border">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-display text-base font-semibold">Add New Employee</h2>
        ...
        <form onSubmit={handleSubmit}>
          <div className="p-6 space-y-4 max-h-[60vh] overflow-y-auto">
            ...
            {/* Step 0: Personal & Job Info */}
            {step === 0 && (
              <>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <Label className="text-xs mb-1.5 block">First Name *</Label>
                    <Input value={form.first_name} onChange={e => set('first_name', e.target.value)} placeholder="e.g. Nati" required />
                  </div>
                  <div>
                    <Label className="text-xs mb-1.5 block">Last Name *</Label>
                    <Input value={form.last_name} onChange={e => set('last_name', e.target.value)} placeholder="e.g. Tekle" required />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <Input type="email" value={form.email} onChange={e => set('email', e.target.value)} placeholder="name@garage.et" />
                  </div>
                </div>
            ...
            {step === 1 && (
              <>
                <div className="p-3.5 rounded-lg border border-border bg-accent/20 text-xs space-y-1">
                  ...
                  <p className="text-muted-foreground">Select payment structure (Monthly vs Weekly) and set basic salary.</p>
                </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <button
                      type="button"
                      onClick={() => {
                        setSal('payment_structure', 'monthly');
                        if (salary.basic_salary_override === '6250') setSal('basic_salary_override', '25000');
                      }}
                    ...
                    <button
                      type="button"
                      onClick={() => {
                        setSal('payment_structure', 'weekly');
                        if (salary.basic_salary_override === '25000') setSal('basic_salary_override', '6250');
                      }}
