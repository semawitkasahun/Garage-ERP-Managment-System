...
const STATS = [
  ...
  { label: "Today's Appointments", value: '14', delta: null, icon: CalendarDays },
...
// TODO: replace with real appointments query for today's date
const TODAY_APPOINTMENTS = [
  { time: '09:00', customer: 'D. Bekele', vehicle: 'Toyota Hilux', service: 'Oil change', bay: 'Bay 2' },
  { time: '10:30', customer: 'H. Girma', vehicle: 'Suzuki Dzire', service: 'Brake inspection', bay: 'Bay 1' },
  { time: '13:00', customer: 'F. Mekonnen', vehicle: 'Hyundai Tucson', service: 'Full service', bay: 'Bay 4' },
  { time: '15:30', customer: 'M. Tesfaye', vehicle: 'Isuzu D-Max', service: 'Diagnostic', bay: 'Bay 3' },
];
    ...
    <DashboardLayout navSections={getNavSections('admin')} pageTitle="Dashboard" roleLabel="Admin">
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        ...
        <div className="rounded-lg border border-border bg-card p-5">
          <h2 className="font-display text-sm font-semibold tracking-tight mb-4">Today's appointments</h2>
          <div className="space-y-3">
            {TODAY_APPOINTMENTS.map((a, i) => (
              <div key={i} className="flex items-start gap-3 text-sm">
                <span className="font-mono text-xs text-muted-foreground w-12 shrink-0 pt-0.5">{a.time}</span>
                <div>
                  <p className="font-medium leading-tight">{a.customer}</p>
                  <p className="text-xs text-muted-foreground leading-tight">{a.vehicle} · {a.service} · {a.bay}</p>
                </div>
              </div>
            ))}
          </div>
            ...
            <thead>
              <tr className="border-b border-border text-left">
                {['ID', 'Vehicle', 'Customer', 'Status', 'Technician', 'Amount'].map((h) => (
                  <th key={h} className="pb-2 font-mono text-[10px] tracking-[0.1em] uppercase text-muted-foreground font-medium">
                    {h}
                  </th>
                ))}
              </tr>
